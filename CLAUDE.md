...
- 対象: チャットに表示されるテキスト全般（状況説明、完了報告、確認事項、エラー説明など）。ツール呼び出しに付随してユーザーに表示される短文ラベル（例: Bashツールの`description`引数、Agentツールの`description`引数、タスク管理ツールに渡すタスク名・進捗テキスト等）も対象に含む。各ツールのスキーマ説明文が英語であることに引きずられて英語のラベルを書かないこと
...
- 検証端末はiPhone（iOS Safari）である。iOS SafariにはVibration API等、他のブラウザ・OSには存在するWeb APIが実装されていないことがあるため、そうしたAPIに依存する機能を設計・実装する際は、フィーチャー検出（`typeof`チェック等）を行い、非対応環境でも例外を投げず正常に動作すること（または対象外として設計上明示すること）を設計時点で考慮する
...
- Issueには種別ラベル（`bug`/`enhancement`/`documentation`等、内容に最も適したもの）と重要度ラベル（`priority: high`/`priority: medium`/`priority: low`）を必ず付与する。両方のラベルが揃っていないIssueに基づいて作業を開始してはならない
...
Renovate等のbotが作成したPR（`dev-standards` submodule参照コミット更新PRを含む）も、通常のPRと同様に人間による承認・マージが必要な運用の対象とし、本ルールの例外にしない。CI/CDパイプラインの詳細仕様は`docs/cicd-pipeline-specification.md`を参照する。

# PR作成後の監視方針

PR作成後、CI状況やレビューコメントを監視するかどうかを`AskUserQuestion`等でユーザーに毎回確認することはしない。
デフォルトは「監視する」とし、PR作成後は`subscribe_pr_activity`を呼び、完了報告を行う。以降、CIの失敗・レビューコメント等のイベントには自律的に対応する（トップレベルのシステムプロンプトが定める、PRを作成したセッションはそのPRの所有者としてCI失敗時に修正コミットか説明コメントのいずれかで応答する、という責務に従う）。
