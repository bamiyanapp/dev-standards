...
- `git filter-repo`・BFG等による履歴書き換え＋force pushは、コミットのSHAを変えるだけである。**マージ済みPRの差分ページ（Files changed）はそのまま残る**。GitHubはPRの削除機能自体を提供していない。削除できるのはIssueではなくリポジトリ全体のみである
...
bamiyanapp/examination Issue #431・#435の事例。想定問答データ（`knowledge/education/interview-*.md`）を調査した。実在の子・保護者の氏名、医療情報、勤務先業種、実際の志望校名・通学先の幼児教室名・保育園名がハードコードされていたことが判明した。汎用プレースホルダーへの置き換えと、家族が自身で登録した氏名をDB経由で表示に反映する仕組み（`app/profile-edit`）へ移行した。この対応中に、削除対象の実際の値をコミットメッセージへ書いてしまうミスも発生しており、本文書の「原則: 個人情報を『消す』コミット・PR自体に実際の値を書かない」はその教訓を反映している。
