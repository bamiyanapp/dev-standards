...
| bamiyanapp/shock-lab | 未対応（対応issue未起票） | 適用済み（[PR #237](https://github.com/bamiyanapp/shock-lab/pull/237)、マージ済み） | `index.css`は`system-ui`等のフォールバックスタックを使用しており、M PLUS Rounded 1c未導入。トップページ必須構成は`vite.config.ts`の`define`でリポジトリルートの`package.json`の`version`・ビルド元コミット日時をビルド時に埋め込み、`AppVersionInfo.tsx`で表示する形で実装。CDのdeployジョブが実際に成功しGitHub Pagesへ反映されたことを確認済み |
