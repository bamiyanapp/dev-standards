...
| bamiyanapp/karuta | 適用済み（[PR #1006](https://github.com/bamiyanapp/karuta/pull/1006)） | 未対応（対応issue未起票） | 共通フォントはkaruta由来のBootstrapテーマ（`shared/ui/bootstrap-theme.css`、issue #232）にfont-family指定が含まれており、PR #1006でkaruta自身もこの共有CSSへ切り替える形で適用済み。トップページのバージョン・更新日時表示はまだ着手していない |
