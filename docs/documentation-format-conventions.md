...
2. `docs/cicd-pipeline-specification.md`の埋め込み例と同じ形式で、mermaidブロック直後（`<details>`で折りたたむ場合はその中）に以下のパターンの画像を埋め込む。
