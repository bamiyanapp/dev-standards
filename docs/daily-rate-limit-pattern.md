...
- `ddb`（DynamoDBClientインスタンス）・`UpdateItemCommand`は呼び出し側で生成・requireして渡す（region等はプロダクト側の都合に委ねる）。このため共有側は`@aws-sdk/client-dynamodb`を含め一切の外部依存を持たない（上記のsymlink+Node.js requireの制約を参照）
