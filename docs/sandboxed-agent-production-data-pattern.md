...
- 対象を確実に絞り込める条件（テーブル名・パーティションキー・特定フィールド名等）が分かっている1回限りの修正では、全件走査ではなく対象を直接指定する方が安全（意図しない行への誤爆を避ける）

## 設計上の要点

- **Job Summaryへ出力してよいのは非機密情報に限る**。bamiyanapp配下のリポジトリは基本的に公開のため、Job Summary・ログの実行履歴は誰でも閲覧できる。秘密情報（認証情報・トークン等）を扱う場合の注意点は`docs/public-repo-secrets-in-actions-pattern.md`を参照する
...
- 1回限りの修正スクリプトは、対応完了後も履歴として残してよい（同種の問題が再発した際の参考実装になる）。ワークフロー名に対象Issue番号を含めておくと、後から経緯を追いやすい
