...
- **実行内容**:
  ...
  - 上記いずれのテストjobも、`coverage_threshold`（グローバルまたは `packages` 内の要素ごと）が0より大きい場合のみ「Check coverage threshold」ステップを実行し、`check-coverage-threshold` 複合アクションで `coverage/coverage-summary.json` を読み、Job Summaryへカバレッジ表を表示し、閾値未満の指標があった場合はジョブを失敗させる。閾値が0（既定）の場合はこのステップ自体を実行しない。**`coverage_threshold`の開発共通標準の目標値は80%以上**（[bamiyanapp/dev-standards#425](https://github.com/bamiyanapp/dev-standards/issues/425)）。`e2e_coverage_threshold`（後述）・`duplication_threshold`と同種の方針で、新規に有効化するリポジトリは実測値が無い導入直後は`0`（ゲート無効・レポート表示のみ）から開始してよいが、実測後にその値のまま据え置いて固定化せず、テストケースを追加してカバレッジ自体を引き上げつつ段階的に閾値を80%へ近づけることを目指す。既存で低い閾値を運用しているリポジトリも、直ちに80%へ上げる必要は無いが、段階的な引き上げ方針を各リポジトリのIssueで管理する。
    ...
    - **失敗4**: 「check-coverage-threshold自体の内容が変わらない限り、多少古いタグのままでも実害はない」という想定のもと、このrefを長期間`v1.2.2`のまま更新していなかった。その間にaction.ymlへ`metrics`/`check-per-file`入力（[dev-standards#57](https://github.com/bamiyanapp/dev-standards/issues/57)）が追加されたが、参照側（karuta等）がこれらの入力を指定しても、checkoutされる古いv1.2.2版のaction.ymlにはその入力定義が無いため、GitHub Actionsは`Unexpected input(s) 'metrics', 'check-per-file'`という警告を出すのみで黙って無視し、絞り込みが一切効かないまま全指標・total平均のみでの判定にフォールバックしていた（[bamiyanapp/karuta#459](https://github.com/bamiyanapp/karuta/issues/459)でcoverage_threshold: 80を有効化した際に発覚）。教訓: **action.ymlの入力を追加・変更した場合は、このrefの更新を同じ変更セットで必ず行うこと**（「内容が変わらなければ実害はない」という前提自体が壊れやすい）

    上記のref更新と同時に、`backend-test`のTestステップへ`--no-isolate`を追加した（同じくbamiyanapp/karuta#459での検証中に発覚）。`@vitest/coverage-v8`は既定（`isolate: true`）でテストファイルごとに新しいモジュールレジストリを使い対象ソースをre-importするため、複数のテストファイルが同一のソースファイルをimportする構成（例: 複数のLambdaハンドラファイルが共通のヘルパー関数を1つのファイルから読み込む）において、カバレッジの集計が壊れることを実測で確認した（`handler.js`単体のテストでは分岐カバレッジ97%だったが、`handler.js`の一部関数を再利用する別のテストファイルが同じ実行に加わるだけで50%まで下がって計測され、実際には満たしている閾値を誤って未達と判定してしまっていた）。当初は「複数ワーカーでの並列実行が原因」という誤った仮説のもと`--no-file-parallelism`（ファイル単位の並列実行を無効化するだけ）を試したが、これでは直らず、真因は`isolate`によるモジュールレジストリの非共有だった。`--no-isolate`で全テストファイルが同じモジュールレジストリを共有するようにしたところ、正しい数値（84%程度）に戻ることを確認した。
  ...
  - `frontend-e2e-test`（任意、`enable_e2e_test: true` の場合のみ）: Playwright による E2E テスト
    ...
    「Show E2E coverage」ステップは`coverage/coverage-summary.json`を読むが、これは参照側リポジトリのPlaywright設定が実際にその形式で出力する構成になっていることが前提であり、単に`enable_e2e_test: true`にしただけでは何も出力されず常にスキップされる。具体的な設定手順は`docs/e2e-coverage-pattern.md`（`monocart-reporter`の導入・`shared/e2e/coverageFixture.js`）を参照する。**`e2e_coverage_threshold`の開発共通標準の目標値は80%以上**（[bamiyanapp/dev-standards#422](https://github.com/bamiyanapp/dev-standards/issues/422)）。新規に有効化する参照側リポジトリは、導入直後は実測値が無いため`0`（ゲート無効・レポート表示のみ）から開始してよいが、実測後にその値のまま据え置いて固定化せず、テストケースを追加してE2Eカバレッジ自体を引き上げつつ、段階的に閾値を80%へ近づけることを目指す（`duplication_threshold`の目標値5%以下・[bamiyanapp/dev-standards#416](https://github.com/bamiyanapp/dev-standards/issues/416)と同種の方針。実測値をそのまま閾値に据え置くラチェット方式だけでは低い状態が固定化されてしまうという、`duplication_threshold`が21%のまま重複の蓄積を検知できなかった[bamiyanapp/examination#447](https://github.com/bamiyanapp/examination/issues/447)の教訓を踏まえる）。既存で低い閾値（`0`・`10`等）を運用している参照側リポジトリは、閾値を直ちに80%へ上げる必要は無いが、段階的な引き上げ方針を各リポジトリのIssueで管理すること（[bamiyanapp/examination#465](https://github.com/bamiyanapp/examination/issues/465)が先行例）。
    ...
    **参照側リポジトリでの呼び出し規約**: `<frontend_dir>/e2e-screenshots/<name>.png`へPNGを書き出すヘルパー関数`captureScreenshot(page, testInfo, name, caption)`を各プロダクトのE2Eテストコード側に用意する。`caption`（第4引数、任意）を渡した場合は同名の`<name>.caption.txt`も書き出し、Job Summary・PRコメントの見出しに日本語等の説明文を使えるようにする。`name`自体は`raw.githubusercontent.com`のURLの一部になるため、英数字のみのASCII安全な識別子にすること（日本語等の非ASCII文字を含めるとURLエンコードの問題が起き得るため、キャプションとして分離する）。このヘルパー関数自体は`shared/e2e/screenshot.js`としてdev-standards側で提供しており（[bamiyanapp/dev-standards#209](https://github.com/bamiyanapp/dev-standards/issues/209)）、`shared/pwa/`・`shared/ui/`と同様にsymlinkで参照側リポジトリへ共有できる（セットアップ方法は`docs/service-worker-update-pattern.md`の`sync-manifest.local.json`の節を参照。参照側のディレクトリ構成に合わせて`{ source: "shared/e2e/screenshot.js", target: "<frontend_dir>/e2e/screenshot.js" }`を追加する）。参照実装（最初の採用例）はkarutaの`frontend/e2e/screenshot.js`。
