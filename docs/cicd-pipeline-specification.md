...
- **実行内容**:
  ...
  - 上記いずれのテストjobも、`coverage_threshold`（グローバルまたは `packages` 内の要素ごと）が0より大きい場合のみ「Check coverage threshold」ステップを実行する。このステップは `check-coverage-threshold` 複合アクションで `coverage/coverage-summary.json` を読み、Job Summaryへカバレッジ表を表示し、閾値未満の指標があった場合はジョブを失敗させる。閾値が0（既定）の場合はこのステップ自体を実行しない。**`coverage_threshold`の開発共通標準の目標値は80%以上**（[bamiyanapp/dev-standards#425](https://github.com/bamiyanapp/dev-standards/issues/425)）。`e2e_coverage_threshold`（後述）・`duplication_threshold`と同種の方針で、新規に有効化するリポジトリは実測値が無い導入直後は`0`（ゲート無効・レポート表示のみ）から開始してよいが、実測後にその値のまま据え置いて固定化せず、テストケースを追加してカバレッジ自体を引き上げつつ段階的に閾値を80%へ近づけることを目指す。既存で低い閾値を運用しているリポジトリも、直ちに80%へ上げる必要は無いが、段階的な引き上げ方針を各リポジトリのIssueで管理する。
    ...
    - **失敗2**: `uses: bamiyanapp/dev-standards/.github/actions/check-coverage-threshold@<ref>`という完全修飾（owner/repo/path@ref）形式に変更し、`<ref>`を直前のステップの出力（`steps.dev-standards-ref.outputs.ref`）から動的に渡す実装を一度リリースした。しかし、**`uses:`フィールドはsteps contextを参照できず、ジョブ計画時に静的に解決可能な値でなければならない**というGitHub Actionsの制約に反していた。その結果、該当ジョブが1つも作成されないまま即座に失敗し（startup failure）、失敗1よりも深刻な状態（CIが実質的に完全停止）を引き起こした
    - **失敗3**: 失敗2を受け、`actions/checkout@v7`（`with:`はsteps contextを問題なく参照できる）で「このワークフロー自身と同じref」のdev-standardsを`.dev-standards-actions/`へcheckoutし、`uses:`は常に静的な相対パス文字列のみにする案に切り替えた。しかし、そのrefを`GITHUB_WORKFLOW_REF`環境変数から導出する実装も誤りだった。**`GITHUB_WORKFLOW_REF`は、このreusable workflow自身のrefではなく、呼び出し元（karuta等）のトップレベルワークフロー自身のref（PRのマージref`refs/pull/<PR番号>/merge`等）を返す**ため、dev-standardsに存在しないrefをcheckoutしようとして失敗した
    ...
    - **失敗4**: 「check-coverage-threshold自体の内容が変わらない限り、多少古いタグのままでも実害はない」という想定のもと、このrefを長期間`v1.2.2`のまま更新していなかった。その間にaction.ymlへ`metrics`/`check-per-file`入力（[dev-standards#57](https://github.com/bamiyanapp/dev-standards/issues/57)）が追加された。しかし参照側（karuta等）がこれらの入力を指定しても、checkoutされる古いv1.2.2版のaction.ymlにはその入力定義が無いため、GitHub Actionsは`Unexpected input(s) 'metrics', 'check-per-file'`という警告を出すのみで黙って無視した。その結果、絞り込みが一切効かないまま全指標・total平均のみでの判定にフォールバックしていた（[bamiyanapp/karuta#459](https://github.com/bamiyanapp/karuta/issues/459)でcoverage_threshold: 80を有効化した際に発覚）。教訓: **action.ymlの入力を追加・変更した場合は、このrefの更新を同じ変更セットで必ず行うこと**（「内容が変わらなければ実害はない」という前提自体が壊れやすい）
    ...
    その後、`backend-test`のTestステップへ`--no-file-parallelism`も追加した。対応の詳細は[bamiyanapp/Electric-Chair-Arena#253](https://github.com/bamiyanapp/Electric-Chair-Arena/pull/253)・[dev-standards#307](https://github.com/bamiyanapp/dev-standards/issues/307)を参照。上記の失敗4の教訓とは別の問題として、`coverage_check_per_file`（issue #57）でファイル単位のカバレッジをゲートする構成において、同一ソースファイルの関数カバレッジがテスト実行のたびに異なる値になる現象を確認した（複数のテストファイルが既定でワーカープロセスに分散実行され、同一ソースのV8カバレッジをプロセス間でマージする際に非決定性が生じると推測される）。その結果、実際には直接・決定的なユニットテストが存在する関数まで偶発的に未カバー扱いとなり、閾値未達でCIが失敗することがあった。デフォルト設定（複数ワーカープロセスに分散）では同一の全テストスイートを繰り返し実行しても毎回異なる関数集合が未カバーと計測されたが、`--no-file-parallelism`で全テストファイルを単一プロセス・単一V8インスタンスに固定したところ、3回連続で完全に同一のカバレッジ値が再現し決定的になることを確認した。上記の失敗4で「`--no-file-parallelism`は本質的な解決にならず」と判断したのは、モジュールレジストリの非共有によるカバレッジ集計崩れに対してであり、本項目（プロセス間マージの非決定性）とは別の問題である点に注意（同じフラグでも解消する問題が異なる）。実行時間は伸びるが、正確な計測を優先する。

    それでもなお`dev-standards#307`が実CI（Electric-Chair-Arena PR #258）で再発したため、V8の「bytecode flushing」（メモリ節約のため、しばらく呼ばれていない関数のbytecodeを自動破棄し次回呼び出し時に再コンパイルする挙動）を無効化する`--no-flush-bytecode`フラグを追加した。当初`NODE_OPTIONS`環境変数へ設定する形でリリースしたが（PR #347）、**`--no-flush-bytecode`はNode.jsがNODE_OPTIONS経由での設定自体を許可しないV8フラグ**だった。そのため`node: --no-flush-bytecode is not allowed in NODE_OPTIONS`（exit code 9）でテストが1件も実行されないままbackend-test全体が即座に壊れることが実CI（Electric-Chair-Arena PR #269）で判明した（リリース前にローカル検証していなかったための見落とし）。同じフラグをNODE_OPTIONSではなく`node`本体への直接のCLI引数として渡す形に修正した（`dev-standards#307`）。具体的には`npx -c 'node --no-flush-bytecode "$(command -v vitest)" run ...'`という形を取り、`npx -c`でnpxに`node_modules/.bin`を含むPATH解決をさせた上で実パスを取得することで、npm workspacesの有無によるhoisting階層差を吸収している。
  ...
  - `text-lint`（任意、`enable_text_lint: true` の場合のみ）: `text_lint_paths`で指定したMarkdownファイル（リポジトリルート基準のglobパターン、カンマ/改行区切りで複数指定可）に対してtextlintを実行するjob（[bamiyanapp/dev-standards#436](https://github.com/bamiyanapp/dev-standards/issues/436)）。`textlint`（`textlint-rule-preset-ja-technical-writing`）で日本語文章の品質チェックを行う。ESLint・stylelintがコード（JS/CSS）向け、CodeQLがセキュリティ向けの静的解析であるのに対し、こちらはMarkdownドキュメントの文章そのもの（文体の不統一・表記ゆれ等）を検知する。textlintの共有設定（`textlint.config.cjs`）はdev-standardsのsubmoduleからsymlinkで配布するため（`docs/code-quality-conventions.md`参照）、このjobは`actions/checkout`時に`submodules: true`を指定する。stylelintと異なり、textlint本体・presetは参照側リポジトリのdevDependenciesへ追加させず、job内で`npx --yes -p textlint@<version> -p textlint-rule-preset-ja-technical-writing@<version>`により都度取得する。これは`duplication-check`の`npx --yes jscpd@5`と同じ方式である。そのため、参照側リポジトリの追加セットアップは`enable_text_lint`・`text_lint_paths`の指定のみで済む。`standards-check`と同様、`frontend_dir`/`backend_dir`に限定されないリポジトリ全体のチェックのため`enable_path_filtering`の対象外（常に実行する）。`merge` jobは他のテストjobと同様にこのjobの成功（またはスキップ）を待つ。
  - `render-mermaid-diagrams`（任意、`enable_mermaid_render: true` の場合のみ）: GitHubのMarkdownビューア自体は```` ```mermaid ```` フェンスをネイティブにレンダリングするが、PR差分ビュー・GitHub CLI/API経由でのファイル取得等では単なるテキストとして表示され図として確認できない（[bamiyanapp/karuta#824](https://github.com/bamiyanapp/karuta/issues/824)）。`mermaid_doc_paths`で指定したMarkdownファイルから```` ```mermaid ```` ブロックを`.github/actions/render-mermaid-diagrams`複合アクション（`@mermaid-js/mermaid-cli`使用）でPNG画像へ事前レンダリングする。Markdown側のmermaidソース自体は書き換えない。
    ...
    mmdcの既定解像度（devicePixelRatio相当）のままだと複雑な図では文字が潰れて読みにくく、またGitHubモバイルアプリの画像ビューアは画像の上下端にヘッダー・フッターのUIを重ねて表示するため図の内容が端まで達していると一部が見切れる問題があった（[bamiyanapp/dev-standards#388](https://github.com/bamiyanapp/dev-standards/issues/388)）。前者は`mmdc`の`-s`（puppeteerのscale factor）オプションで出力PNGの物理ピクセル数を上げることで、後者はmmdc実行後にImageMagickの`convert`で白背景の余白（上下は特に広め）を追加することで解消している。
...
**複数アプリ向けに`reusable-ci.yml`を独立して複数回呼び出す構成（1 job per appパターン）でのマージ（[issue #421](https://github.com/bamiyanapp/dev-standards/issues/421)）**: `frontend_dir`はmatrix非対応でスカラー値のみのため、E2E対象アプリが複数ある場合、参照側の`ci.yml`が`reusable-ci.yml`を独立した複数のjob（例: `ci`・`e2e-app2`）として呼び出す構成を取ることがある。この場合、各呼び出しがそれぞれ**別々の`merge` job**を持つため、`enable_auto_merge: true`のままだと、片方の呼び出しが自身の完全な成功を確認して先にマージしてしまい、もう片方がまだ失敗中・実行中でもPRがマージされてしまう。この構成を取る場合は、以下のようにする。

1. `reusable-ci.yml`への全呼び出しに`enable_auto_merge: false`を指定する（各呼び出し自身のmerge jobを無効化する）
2. 呼び出し側の`ci.yml`に、全呼び出しの完了を`needs`で待つ専用のgate jobを追加し、`.github/actions/merge-pr`複合アクションを`uses: bamiyanapp/dev-standards/.github/actions/merge-pr@<tag>`（完全修飾形式、静的なタグ文字列を指定）で直接呼び出す。`with: github-token`には`merge` jobと同様に`BOT_TOKEN`（未設定時は`GITHUB_TOKEN`）を渡す
...
入力パラメータの全体は`docs/reusable-workflows-reference.md`を参照する。semantic-releaseの実行に関する入力は`reusable-cd.yml`側にある。具体的には`enable_release` / `semantic_release_node_version` / `base_branch` / `enable_changelog_json` / `changelog_source_path` / `changelog_json_output_path` / `enable_shared_release_config`である。このワークフロー（`reusable-ci.yml`）自体には存在しない。[bamiyanapp/dev-standards#76](https://github.com/bamiyanapp/dev-standards/issues/76)でのメジャーバージョンアップに伴い削除した。
...
入力パラメータ（`enable_release` / `semantic_release_node_version` / `enable_shared_release_config` / `enable_changelog_json` / `changelog_source_path` / `changelog_json_output_path`）がある。詳細は docs/reusable-workflows-reference.md を参照。
   ...
   deploy:
     ...
     steps:
       - run: |
           ...
           npm run build
   ```
2. **ビルドツールの規約に沿った環境変数名にする**: クライアントサイドのJSバンドルへ埋め込むには、多くのビルドツールが「特定のプレフィックスを持つ環境変数のみをバンドルへ含める」制約を設けている（例: Viteは`VITE_`、Next.jsは`NEXT_PUBLIC_`、Create React Appは`REACT_APP_`）。プレフィックスの無い環境変数はビルド成果物に含まれず`undefined`になるため、使用するビルドツールの規約を確認すること。
