...
- **実行内容**:
  ...
  - 上記いずれのテストjobも、`coverage_threshold`（グローバルまたは `packages` 内の要素ごと）が0より大きい場合のみ「Check coverage threshold」ステップを実行し、`check-coverage-threshold` 複合アクションで `coverage/coverage-summary.json` を読み、Job Summaryへカバレッジ表を表示し、閾値未満の指標があった場合はジョブを失敗させる。閾値が0（既定）の場合はこのステップ自体を実行しない。**`coverage_threshold`の開発共通標準の目標値は80%以上**（[bamiyanapp/dev-standards#425](https://github.com/bamiyanapp/dev-standards/issues/425)）。`e2e_coverage_threshold`（後述）・`duplication_threshold`と同種の方針で、新規に有効化するリポジトリは実測値が無い導入直後は`0`（ゲート無効・レポート表示のみ）から開始してよいが、実測後にその値のまま据え置いて固定化せず、テストケースを追加してカバレッジ自体を引き上げつつ段階的に閾値を80%へ近づけることを目指す。既存で低い閾値を運用しているリポジトリも、直ちに80%へ上げる必要は無いが、段階的な引き上げ方針を各リポジトリのIssueで管理する。
    ...
    その後、`backend-test`のTestステップへ`--no-file-parallelism`も追加した（[bamiyanapp/Electric-Chair-Arena#253](https://github.com/bamiyanapp/Electric-Chair-Arena/pull/253), [dev-standards#307](https://github.com/bamiyanapp/dev-standards/issues/307)）。上記の失敗4の教訓とは別の問題: `coverage_check_per_file`（issue #57）でファイル単位のカバレッジをゲートする構成において、同一ソースファイルの関数カバレッジがテスト実行のたびに異なる値になり（複数のテストファイルが既定でワーカープロセスに分散実行され、同一ソースのV8カバレッジをプロセス間でマージする際に非決定性が生じると推測される）、実際には直接・決定的なユニットテストが存在する関数まで偶発的に未カバー扱いとなり閾値未達でCIが失敗する現象を確認した。デフォルト設定（複数ワーカープロセスに分散）では同一の全テストスイートを繰り返し実行しても毎回異なる関数集合が未カバーと計測されたが、`--no-file-parallelism`で全テストファイルを単一プロセス・単一V8インスタンスに固定したところ、3回連続で完全に同一のカバレッジ値が再現し決定的になることを確認した。上記の失敗4で「`--no-file-parallelism`は本質的な解決にならず」と判断したのは、モジュールレジストリの非共有によるカバレッジ集計崩れに対してであり、本項目（プロセス間マージの非決定性）とは別の問題である点に注意（同じフラグでも解消する問題が異なる）。実行時間は伸びるが、正確な計測を優先する。

    それでもなお`dev-standards#307`が実CI（Electric-Chair-Arena PR #258）で再発したため、V8の「bytecode flushing」（メモリ節約のため、しばらく呼ばれていない関数のbytecodeを自動破棄し次回呼び出し時に再コンパイルする挙動）を無効化する`--no-flush-bytecode`フラグを追加した。当初`NODE_OPTIONS`環境変数へ設定する形でリリースしたが（PR #347）、**`--no-flush-bytecode`はNode.jsがNODE_OPTIONS経由での設定自体を許可しないV8フラグ**であり、`node: --no-flush-bytecode is not allowed in NODE_OPTIONS`（exit code 9）でテストが1件も実行されないままbackend-test全体が即座に壊れることが実CI（Electric-Chair-Arena PR #269）で判明した（リリース前にローカル検証していなかったための見落とし）。同じフラグをNODE_OPTIONSではなく`node`本体への直接のCLI引数として渡す形（`npx -c 'node --no-flush-bytecode "$(command -v vitest)" run ...'`、`npx -c`でnpxに`node_modules/.bin`を含むPATH解決をさせた上で実パスを取得することで、npm workspacesの有無によるhoisting階層差を吸収している）に修正した（`dev-standards#307`）。
  ...
  - `text-lint`（任意、`enable_text_lint: true` の場合のみ）: `text_lint_paths`で指定したMarkdownファイル（リポジトリルート基準のglobパターン、カンマ/改行区切りで複数指定可）に対し、`textlint`（`textlint-rule-preset-ja-technical-writing`）で日本語文章の品質チェックを行う（[bamiyanapp/dev-standards#436](https://github.com/bamiyanapp/dev-standards/issues/436)）。ESLint・stylelintがコード（JS/CSS）向け、CodeQLがセキュリティ向けの静的解析であるのに対し、こちらはMarkdownドキュメントの文章そのもの（文体の不統一・表記ゆれ等）を検知する。textlintの共有設定（`textlint.config.cjs`）はdev-standardsのsubmoduleからsymlinkで配布するため（`docs/code-quality-conventions.md`参照）、このjobは`actions/checkout`時に`submodules: true`を指定する。stylelintと異なり、textlint本体・presetは参照側リポジトリのdevDependenciesへ追加させず、job内で`npx --yes -p textlint@<version> -p textlint-rule-preset-ja-technical-writing@<version>`により都度取得する（`duplication-check`の`npx --yes jscpd@5`と同じ方式）ため、参照側リポジトリの追加セットアップは`enable_text_lint`・`text_lint_paths`の指定のみで済む。`standards-check`と同様、`frontend_dir`/`backend_dir`に限定されないリポジトリ全体のチェックのため`enable_path_filtering`の対象外（常に実行する）。`merge` jobは他のテストjobと同様にこのjobの成功（またはスキップ）を待つ。
...
入力パラメータの全体は`docs/reusable-workflows-reference.md`を参照する。semantic-releaseの実行に関する入力（`enable_release` / `semantic_release_node_version` / `base_branch` / `enable_changelog_json` / `changelog_source_path` / `changelog_json_output_path` / `enable_shared_release_config`）は`reusable-cd.yml`側にあり、このワークフローには存在しない。[bamiyanapp/dev-standards#76](https://github.com/bamiyanapp/dev-standards/issues/76)でのメジャーバージョンアップに伴い削除した。
