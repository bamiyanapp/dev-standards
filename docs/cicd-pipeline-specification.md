...
上記の```mermaid```ブロックはPR差分ビュー・API経由でのファイル取得等ではテキストのまま表示され図として確認できない（[bamiyanapp/karuta#824](https://github.com/bamiyanapp/karuta/issues/824)）。ソース（mermaid記法）はこのまま維持する。下記は`enable_mermaid_render`（`render-mermaid-diagrams` job）が`base_branch`へのマージのたびに再レンダリングし、`docs-diagrams`ブランチの`latest/`へ上書き公開している画像（常に最新版）。
...
- **実行内容**:
  ...
  - 上記いずれのテストjobも、`coverage_threshold`（グローバルまたは `packages` 内の要素ごと）が0より大きい場合のみ「Check coverage threshold」ステップを実行する。このステップは `check-coverage-threshold` 複合アクションで `coverage/coverage-summary.json` を読み、Job Summaryへカバレッジ表を表示し、閾値未満の指標があった場合はジョブを失敗させる。閾値が0（既定）の場合はこのステップ自体を実行しない。**`coverage_threshold`の開発共通標準の目標値は80%以上**（[bamiyanapp/dev-standards#425](https://github.com/bamiyanapp/dev-standards/issues/425)）。`e2e_coverage_threshold`（後述）・`duplication_threshold`と同種の方針で、新規に有効化するリポジトリは実測値が無い導入直後は`0`（ゲート無効・レポート表示のみ）から開始してよい。ただし実測後にその値のまま据え置いて固定化せず、テストケースを追加してカバレッジ自体を引き上げつつ段階的に閾値を80%へ近づけることを目指す。既存で低い閾値を運用しているリポジトリも、直ちに80%へ上げる必要は無いが、段階的な引き上げ方針を各リポジトリのIssueで管理する。
    ...
    - **失敗4**: 「check-coverage-threshold自体の内容が変わらない限り、多少古いタグのままでも実害はない」という想定のもと、このrefを長期間`v1.2.2`のまま更新していなかった。その間にaction.ymlへ`metrics`/`check-per-file`入力（[dev-standards#57](https://github.com/bamiyanapp/dev-standards/issues/57)）が追加された。しかし参照側（karuta等）がこれらの入力を指定しても、checkoutされる古いv1.2.2版のaction.ymlにはその入力定義が無い。このためGitHub Actionsは`Unexpected input(s) 'metrics', 'check-per-file'`という警告を出すのみで黙って無視した。その結果、絞り込みが一切効かないまま全指標・total平均のみでの判定にフォールバックしていた（[bamiyanapp/karuta#459](https://github.com/bamiyanapp/karuta/issues/459)でcoverage_threshold: 80を有効化した際に発覚）。教訓: **action.ymlの入力を追加・変更した場合は、このrefの更新を同じ変更セットで必ず行うこと**（「内容が変わらなければ実害はない」という前提自体が壊れやすい）

    上記のref更新と同時に、`backend-test`のTestステップへ`--no-isolate`を追加した（同じくbamiyanapp/karuta#459での検証中に発覚）。`@vitest/coverage-v8`は既定（`isolate: true`）でテストファイルごとに新しいモジュールレジストリを使い対象ソースをre-importする。そのため、複数のテストファイルが同一のソースファイルをimportする構成（例: 複数のLambdaハンドラファイルが共通のヘルパー関数を1つのファイルから読み込む）において、カバレッジの集計が壊れることを実測で確認した。具体的には、`handler.js`単体のテストでは分岐カバレッジ97%だったが、`handler.js`の一部関数を再利用する別のテストファイルが同じ実行に加わるだけで50%まで下がって計測され、実際には満たしている閾値を誤って未達と判定してしまっていた。当初は「複数ワーカーでの並列実行が原因」という誤った仮説のもと`--no-file-parallelism`（ファイル単位の並列実行を無効化するだけ）を試したが、これでは直らず、真因は`isolate`によるモジュールレジストリの非共有だった。`--no-isolate`で全テストファイルが同じモジュールレジストリを共有するようにしたところ、正しい数値（84%程度）に戻ることを確認した。

    ただし`--no-isolate`は`frontend-test`には適用していない。frontendのテストスイートはモジュールスコープの状態（共有`<audio>`要素のシングルトン等）がテストファイルごとに独立している前提で書かれており、`--no-isolate`を適用すると54件のテストが失敗した。参照側リポジトリでこの種の対象外にしたいモジュールスコープの状態がある場合は、`--no-isolate`の適用自体を見直す必要がある。

    その後、`backend-test`のTestステップへ`--no-file-parallelism`も追加した。対応の詳細は[bamiyanapp/Electric-Chair-Arena#253](https://github.com/bamiyanapp/Electric-Chair-Arena/pull/253)・[dev-standards#307](https://github.com/bamiyanapp/dev-standards/issues/307)を参照。上記の失敗4の教訓とは別の問題として、`coverage_check_per_file`（issue #57）でファイル単位のカバレッジをゲートする構成において、同一ソースファイルの関数カバレッジがテスト実行のたびに異なる値になる現象を確認した。複数のテストファイルが既定でワーカープロセスに分散実行され、同一ソースのV8カバレッジをプロセス間でマージする際に非決定性が生じると推測される。その結果、実際には直接・決定的なユニットテストが存在する関数まで偶発的に未カバー扱いとなり、閾値未達でCIが失敗することがあった。デフォルト設定（複数ワーカープロセスに分散）では同一の全テストスイートを繰り返し実行しても毎回異なる関数集合が未カバーと計測された。`--no-file-parallelism`で全テストファイルを単一プロセス・単一V8インスタンスに固定したところ、3回連続で完全に同一のカバレッジ値が再現し決定的になることを確認した。上記の失敗4で「`--no-file-parallelism`は本質的な解決にならず」と判断したのは、モジュールレジストリの非共有によるカバレッジ集計崩れに対してであり、本項目（プロセス間マージの非決定性）とは別の問題である点に注意（同じフラグでも解消する問題が異なる）。実行時間は伸びるが、正確な計測を優先する。

    それでもなお`dev-standards#307`が実CI（Electric-Chair-Arena PR #258）で再発した。そこでV8の「bytecode flushing」（メモリ節約のため、しばらく呼ばれていない関数のbytecodeを自動破棄し次回呼び出し時に再コンパイルする挙動）を無効化する`--no-flush-bytecode`フラグを追加した。当初`NODE_OPTIONS`環境変数へ設定する形でリリースしたが（PR #347）、**`--no-flush-bytecode`はNode.jsがNODE_OPTIONS経由での設定自体を許可しないV8フラグ**だった。そのため`node: --no-flush-bytecode is not allowed in NODE_OPTIONS`（exit code 9）でテストが1件も実行されなかった。backend-test全体が即座に壊れることが実CI（Electric-Chair-Arena PR #269）で判明した。リリース前にローカル検証していなかったための見落としである。同じフラグをNODE_OPTIONSではなく`node`本体への直接のCLI引数として渡す形に修正した（`dev-standards#307`）。具体的には`npx -c 'node --no-flush-bytecode "$(command -v vitest)" run ...'`という形を取る。`npx -c`でnpxに`node_modules/.bin`を含むPATH解決をさせた上で実パスを取得することで、npm workspacesの有無によるhoisting階層差を吸収している。
  ...
  - `frontend-e2e-test`（任意、`enable_e2e_test: true` の場合のみ）: Playwright による E2E テスト
    ...
    「Show E2E coverage」ステップは`coverage/coverage-summary.json`を読む。これは参照側リポジトリのPlaywright設定が実際にその形式で出力する構成になっていることが前提であり、単に`enable_e2e_test: true`にしただけでは何も出力されず常にスキップされる。具体的な設定手順は`docs/e2e-coverage-pattern.md`（`monocart-reporter`の導入・`shared/e2e/coverageFixture.js`）を参照する。**`e2e_coverage_threshold`の開発共通標準の目標値は80%以上**（[bamiyanapp/dev-standards#422](https://github.com/bamiyanapp/dev-standards/issues/422)）。新規に有効化する参照側リポジトリは、導入直後は実測値が無いため`0`（ゲート無効・レポート表示のみ）から開始してよい。ただし実測後にその値のまま据え置いて固定化せず、テストケースを追加してE2Eカバレッジ自体を引き上げつつ、段階的に閾値を80%へ近づけることを目指す。これは`duplication_threshold`の目標値5%以下（[bamiyanapp/dev-standards#416](https://github.com/bamiyanapp/dev-standards/issues/416)）と同種の方針である。実測値をそのまま閾値に据え置くラチェット方式だけでは低い状態が固定化されてしまう。`duplication_threshold`が21%のまま重複の蓄積を検知できなかった[bamiyanapp/examination#447](https://github.com/bamiyanapp/examination/issues/447)の教訓を踏まえたものである。既存で低い閾値（`0`・`10`等）を運用している参照側リポジトリは、閾値を直ちに80%へ上げる必要は無いが、段階的な引き上げ方針を各リポジトリのIssueで管理すること（[bamiyanapp/examination#465](https://github.com/bamiyanapp/examination/issues/465)が先行例）。
...
```
    ...
    **前回バージョンとの画像diffによる添付省略（[bamiyanapp/karuta#750](https://github.com/bamiyanapp/karuta/issues/750)）**: 呼び出し側の追加対応は不要（`<name>.png`を書き出す既存の規約のみで動作する）。`.github/actions/compare-e2e-screenshots`複合アクション（`pixelmatch`・`pngjs`を使用、PRのみ実行）を使う。新しいスクリーンショットと`e2e-screenshots`ブランチの`latest/`配下（直近の`base_branch`時点）の同名PNGをピクセル単位で比較する。寸法が異なる場合や1ピクセルでも差があれば`"changed"`、完全一致なら`"unchanged"`、`latest/`に同名ファイルが無ければ`"new"`と判定し、`"unchanged"`と判定されたスクリーンショットのみPRコメント・Job Summaryへの添付を省略する。比較対象（`latest/`）自体は`push`イベント（`base_branch`へのマージ後）のたびに上書き公開されるため、常に「直近のbase_branchと比べて見た目が変わったかどうか」を表す。閾値判定ではなく単純な視覚的差分の有無のみを見るため、意図した見た目の変更であってもスクリーンショットは通常どおり表示される（レビュー時に見るべき差分を減らすのが目的であり、変更の可否を自動判定するものではない）。
    ...
    `shared/e2e/screenshot.js`はsymlink共有のため、`shared/pwa/`・`shared/ui/`と同様にバージョン固定の仕組みは無く、参照側リポジトリの`dev-standards`サブモジュール参照コミットを更新した時点で変更が反映される。プロダクト固有の値を持たない汎用コードであることを維持し、あるプロダクト向けの事情でこのファイルへ分岐を持ち込まないこと。プロダクト固有のカスタマイズが必要になった場合は、`docs/service-worker-update-pattern.md`の`UpdateNotifier.jsx`の節で述べているのと同様に、symlink化を見送りコピーして個別管理する。

    **認証を伴うE2Eでの参照側リポジトリ固有secretsの受け渡し（`E2E_SECRETS_JSON`、任意、[issue #371](https://github.com/bamiyanapp/dev-standards/issues/371)）**: `docs/client-only-vite-spa-pattern.md`が定める「E2Eはモックを作らず実際のAPIへ直結する」原則がある。この仕組みは、ログイン等の対話的な同意画面を伴うE2Eでもその原則を維持できるようにする。呼び出し元が`secrets.E2E_SECRETS_JSON`へ`{"KEY1":"value1","KEY2":"value2"}`形式のJSON文字列を渡すと、「Run E2E tests」ステップの直前でこれをパースする。値ごとに`::add-mask::`でログのマスク対象として個別登録した上で`$GITHUB_ENV`へ書き出す（JSON全体を1つのsecretとして登録するだけでは、パース後の個々の値がログのマスク対象にならないため）。キー名は`reusable-ci.yml`側では一切関知せず、参照側リポジトリが`npm run test:e2e`内で`process.env.<KEY>`として自由に参照できる。想定用途は、対話的なOAuth同意画面をPlaywrightで自動操作する代わりに、CI実行時にリフレッシュトークン等から事前に取得した実アクセストークンをブラウザへ注入し、その先のAPI呼び出しは実際のAPIへ直結させることである。同意画面の自動操作という不可能な部分だけを迂回し、それ以外は実APIに直結する設計である。未設定時は何も行わない（後方互換）。
  - `duplication-check`（任意、`enable_duplication_check: true` の場合のみ）: SonarCloud相当の静的解析をCIネイティブなツールの組み合わせで代替する取り組みの一部（[bamiyanapp/karuta#806](https://github.com/bamiyanapp/karuta/issues/806)）。`jscpd`（Rust製、依存なし）でリポジトリ全体（`.gitignore`に従い`node_modules`等を除外、javascript/jsx/typescript/tsxのみ対象）のコード重複を検知し、markdownレポーターの出力をJob Summaryへそのまま表示する。テストファイル（`*.test.*`・`*.spec.*`・`e2e`ディレクトリ配下）は`-i`で除外し、プロダクトコードのみを判定対象にする（[bamiyanapp/karuta#833](https://github.com/bamiyanapp/karuta/issues/833)）。テストコードの重複はセットアップ・アサーションの意図的な繰り返しであることが多く、プロダクトコードの重複とは性質が異なるため対象から外す。`duplication_threshold`が0（既定）の場合はレポート表示のみでゲートしない。0より大きい値を指定すると`jscpd`の`--threshold`により、重複率がその値を超えた場合にjob自体を失敗させる。`merge` jobは他のテストジョブと同様にこのjobの成功（またはスキップ）を待つ。**`duplication_threshold`の開発共通標準の目標値は5%以下**（[bamiyanapp/dev-standards#416](https://github.com/bamiyanapp/dev-standards/issues/416)）。新規に有効化する参照側リポジトリは、導入時点の実測値が5%を超えていてもその値をそのまま閾値へ据え置かず、共通化・symlink化等で実際に重複を減らした上で5%以下を目指す。導入時点の実測値をそのまま閾値にするラチェット方式（karuta#806・#833・#877）だけでは、重複の多い状態がそのまま固定化されてしまう。[bamiyanapp/examination#447](https://github.com/bamiyanapp/examination/issues/447)で、`duplication_threshold: 21`のまま重複が蓄積し検知できていなかったことが判明した教訓を踏まえている。既に高い閾値を運用している既存リポジトリは、直ちに5%へ下げる必要は無いが、段階的な削減方針を各リポジトリのIssueで管理すること。

    SonarCloudが持つ「複雑度（cyclomatic complexity）」「コードスメル・保守性（cognitive complexity等）」の代替（ESLintの`complexity`ルール・`eslint-plugin-sonarjs`）は存在する。dev-standardsが共有ESLint設定を提供していないため、このワークフローでは扱わない。参照側リポジトリの各パッケージの`eslint.config.js`へ直接追加すること。
  - `doc-duplication-check`（任意、`enable_doc_duplication_check: true` の場合のみ）: Markdownドキュメント間の丸ごとコピペ重複を検知する（[bamiyanapp/dev-standards#586](https://github.com/bamiyanapp/dev-standards/issues/586)）。`duplication-check`（コード向け）とは別のjobである。CLAUDE.mdと各`SKILL.md`で同じ禁止ルールを重複記載し、後から片方だけ更新されて文言がずれる（drift）事故が実際に発生したことが動機。`doc_duplication_paths`（カンマ/改行区切り、ディレクトリまたはファイルパス）で指定した対象へ`jscpd@5`をそのまま実行する。`duplication-check`と異なり`-f`によるフォーマット限定は使わない。`jscpd`の`-f markdown`は対象拡張子をmarkdownへ限定するのではなくリポジトリ全体を素通しで検知してしまう挙動が実機検証で確認されたため、対象パス自体を`doc_duplication_paths`で絞り込む方式にした。ドキュメント内の埋め込みコードサンプル（```` ```js ````等のフェンス）は`jscpd`の自動判定により別フォーマットとして計上されるため、意図的に複数の完全なコード例を並べているドキュメントがある場合はその分の重複が閾値に乗る。トークン単位の厳密一致のみを判定するため、コピペではなく言い回しだけを変えた重複（パラフレーズ）は検知できない点も限界として明記する。`doc_duplication_threshold`が0（既定）の場合はレポート表示のみでゲートしない。`merge` jobは他のテストジョブと同様にこのjobの成功（もしくはスキップ）を待つ。
  - `architecture-check`（任意、`enable_architecture_check: true` の場合のみ）: `complexity`・`sonarjs`（循環的複雑度）、`jscpd`（コード重複）に対する検知はあった。しかしモジュール間の依存方向（アーキテクチャ）違反を検知する仕組みが無かった（[bamiyanapp/dev-standards#523](https://github.com/bamiyanapp/dev-standards/issues/523)）。`dependency-cruiser`で循環依存・`frontend_dir`/`backend_dir`間の越境import（frontend側からbackend内部モジュールへの直接import等）を検知する。npm workspacesモノレポでは、GitHub側の機構だけではこの越境importを物理的に禁止できず、AI駆動開発（差分単位でしか見ない）では層違反がレビューをすり抜けて蓄積しやすい。共有ルール（`dependency-cruiser.config.cjs`）はdev-standardsのsubmoduleからsymlinkで配布する（`docs/code-quality-conventions.md`参照）。このためこのjobは`actions/checkout`時に`submodules: true`を指定する。`text-lint`と同様、dependency-cruiser本体は参照側リポジトリのdevDependenciesへ追加させず、job内で`npx --yes dependency-cruiser@16`により都度取得する。検知対象は`frontend_dir`・`backend_dir`のうち実在するディレクトリに限定し、単一パッケージ構成では越境importルールが該当せず循環依存の検知のみが働く。`merge` jobは他のテストjobと同様にこのjobの成功（またはスキップ）を待つ。
  - `text-lint`（任意、`enable_text_lint: true` の場合のみ）: `text_lint_paths`で指定したMarkdownファイル（リポジトリルート基準のglobパターン、カンマ/改行区切りで複数指定可）に対してtextlintを実行するjob。詳細は[bamiyanapp/dev-standards#436](https://github.com/bamiyanapp/dev-standards/issues/436)を参照。`textlint`（`textlint-rule-preset-ja-technical-writing`）で日本語文章の品質チェックを行う。ESLint・stylelintがコード（JS/CSS）向け、CodeQLがセキュリティ向けの静的解析であるのに対し、こちらはMarkdownドキュメントの文章そのもの（文体の不統一・表記ゆれ等）を検知する。textlintの共有設定（`textlint.config.cjs`）はdev-standardsのsubmoduleからsymlinkで配布する（`docs/code-quality-conventions.md`参照）。このためこのjobは`actions/checkout`時に`submodules: true`を指定する。stylelintと異なり、textlint本体・presetは参照側リポジトリのdevDependenciesへ追加させない。job内で`npx --yes -p textlint@<version> -p textlint-rule-preset-ja-technical-writing@<version>`により都度取得する。これは`duplication-check`の`npx --yes jscpd@5`と同じ方式である。そのため、参照側リポジトリの追加セットアップは`enable_text_lint`・`text_lint_paths`の指定のみで済む。`standards-check`と同様、`frontend_dir`/`backend_dir`に限定されないリポジトリ全体のチェックのため`enable_path_filtering`の対象外（常に実行する）。`merge` jobは他のテストjobと同様にこのjobの成功（またはスキップ）を待つ。
  - `render-mermaid-diagrams`（任意、`enable_mermaid_render: true` の場合のみ）: GitHubのMarkdownビューア自体は```` ```mermaid ```` フェンスをネイティブにレンダリングする。しかしPR差分ビュー・GitHub CLI/API経由でのファイル取得等では単なるテキストとして表示され図として確認できない（[bamiyanapp/karuta#824](https://github.com/bamiyanapp/karuta/issues/824)）。`mermaid_doc_paths`で指定したMarkdownファイルから```` ```mermaid ```` ブロックを`.github/actions/render-mermaid-diagrams`複合アクション（`@mermaid-js/mermaid-cli`使用）でPNG画像へ事前レンダリングする。Markdown側のmermaidソース自体は書き換えない。
    ...
    GitHub Actionsの`ubuntu-latest`ランナーには日本語（CJK）フォントが既定でインストールされておらず、レンダリング結果の日本語ラベルが文字化け（tofu表示、`□`の連続）する問題があった（[bamiyanapp/karuta#849](https://github.com/bamiyanapp/karuta/issues/849)）。そのため、`render-mermaid-diagrams`複合アクション内で`mmdc`実行前に`fonts-noto-cjk`パッケージを`apt-get install`している。

    mmdcの既定解像度（devicePixelRatio相当）のままだと複雑な図では文字が潰れて読みにくい。またGitHubモバイルアプリの画像ビューアは画像の上下端にヘッダー・フッターのUIを重ねて表示するため、図の内容が端まで達していると一部が見切れる問題があった（[bamiyanapp/dev-standards#388](https://github.com/bamiyanapp/dev-standards/issues/388)）。前者は`mmdc`の`-s`（puppeteerのscale factor）オプションで出力PNGの物理ピクセル数を上げることで、後者はmmdc実行後にImageMagickの`convert`で白背景の余白（上下は特に広め）を追加することで解消している。

    公開先は2段階（E2Eスクリーンショット報告機能と同じ構成）。`runs/<run_id>/`配下へは実行のたびに公開し、Job Summary・PRコメント（`pull_request`イベントのみ）へその場限りの確認用として埋め込む。加えて、`push`イベント（`base_branch`へのマージ後）のたびに`latest/`配下へ上書き公開する。こちらのURL（`https://raw.githubusercontent.com/<repo>/docs-diagrams/latest/<ファイル名>`）を対象Markdownファイル自身の```` ```mermaid ```` ブロック直後へ`![...](...)`として恒久的に埋め込んでおく。これにより、PRコメントというその場限りの確認手段だけでなく、ドキュメント本体を開いた際にも常に最新のレンダリング結果を確認できる。埋め込みは最初の1回のみ手動で行えばよく、以後は`latest/`側の画像内容が自動更新されるためMarkdown側の再編集は不要である。
...
**複数アプリ向けに`reusable-ci.yml`を独立して複数回呼び出す構成（1 job per appパターン）でのマージ（[issue #421](https://github.com/bamiyanapp/dev-standards/issues/421)）**: `frontend_dir`はmatrix非対応でスカラー値のみである。E2E対象アプリが複数ある場合、参照側の`ci.yml`が`reusable-ci.yml`を独立した複数のjob（例: `ci`・`e2e-app2`）として呼び出す構成を取ることがある。この場合、各呼び出しがそれぞれ**別々の`merge` job**を持つため、`enable_auto_merge: true`のままだと、片方の呼び出しが自身の完全な成功を確認して先にマージしてしまい、もう片方がまだ失敗中・実行中でもPRがマージされてしまう。この構成を取る場合は、以下のようにする。

1. `reusable-ci.yml`への全呼び出しに`enable_auto_merge: false`を指定する（各呼び出し自身のmerge jobを無効化する）
2. 呼び出し側の`ci.yml`に、全呼び出しの完了を`needs`で待つ専用のgate jobを追加する。このjobから`.github/actions/merge-pr`複合アクションを`uses: bamiyanapp/dev-standards/.github/actions/merge-pr@<tag>`（完全修飾形式、静的なタグ文字列を指定）で直接呼び出す。`with: github-token`には`merge` jobと同様に`BOT_TOKEN`（未設定時は`GITHUB_TOKEN`）を渡す
3. gate jobの`if:`条件で、全呼び出しの各jobの結果（`needs.<job>.result`）を`reusable-ci.yml`内部の`merge` jobと同じ方式で判定する（`skipped`は許容し`failure`・`cancelled`は拒否する）

**`skip_verification_on_push`（任意、既定`false`）**: CI/CD実行回数・実行時間削減バックログ（[bamiyanapp/dev-standards#187](https://github.com/bamiyanapp/dev-standards/issues/187)③）の対応。`true`かつ`push`イベント（`base_branch`への直接push、Squash merge後の`push-to-main`等）の場合、`frontend-test`・`backend-test`・`package-test`をスキップする。`standards-check`・`duplication-check`・`doc-duplication-check`・`architecture-check`もスキップする。これらのjobは、PRが「`base_branch`と同期済みでなければマージ不可」（up-to-date required）という前提のもとSquash mergeされる運用を前提にしている。この運用では、マージ後のmain上のツリーが直前の`pull_request`イベントで検証済みのPR head内容と完全に一致するため、push側での再実行が構造的に冗長であることを利用した最適化である。**この前提（up-to-date必須化＋Squash merge運用）が成り立たない参照側リポジトリでは有効化しないこと**。`frontend-e2e-test`（E2Eスクリーンショットの`latest/`ベースライン公開、前述）と`render-mermaid-diagrams`（mermaid画像の`latest/`公開、前述）は、いずれも`push`イベントでのみ発生する副作用（後続のPRが参照するベースラインの更新）を担う。そのため、このスキップ対象には含めない。

**`enable_path_filtering`（任意、既定`false`）**: CI/CD実行回数・実行時間削減バックログ（[bamiyanapp/dev-standards#187](https://github.com/bamiyanapp/dev-standards/issues/187)⑤）の対応。`true`の場合、新規`detect-changes` jobが`dorny/paths-filter@v3`で`frontend_dir`/`backend_dir`配下の変更有無を検出する。`frontend-test`・`backend-test`・`frontend-e2e-test`・`duplication-check`・`architecture-check`は、それぞれ該当する変更が無い場合にスキップされる。ルート直下の`package.json`・`package-lock.json`・`.github/workflows/**`の変更はfrontend/backend双方の「変更あり」とみなす（共通依存・ワークフロー自体の変更時に必要なチェックを取りこぼさないため）。`packages`入力使用時（package-testモード）はパッケージ単位の変更検出が未対応のため対象外とし、常に全jobを実行する。`standards-check`（`sync-manifest.json`に基づくリポジトリ全体のsymlink整合性チェック）・`doc-duplication-check`（Markdownドキュメント重複検知）は`frontend_dir`/`backend_dir`に限定されないため対象外とし常に実行する。`detect-changes` job自体の実行が失敗した場合（GitHub側の一時的な障害等）は安全側に倒し、frontend/backendとも「変更あり」として全jobを実行する。

入力パラメータの全体は`docs/reusable-workflows-reference.md`を参照する。semantic-releaseの実行に関する入力は`reusable-cd.yml`側にある。具体的には`enable_release` / `semantic_release_node_version` / `base_branch`が該当する。加えて`enable_changelog_json` / `changelog_source_path` / `changelog_json_output_path` / `enable_shared_release_config`も該当する。このワークフロー（`reusable-ci.yml`）自体には存在しない。[bamiyanapp/dev-standards#76](https://github.com/bamiyanapp/dev-standards/issues/76)でのメジャーバージョンアップに伴い削除した。
...
- `push`イベント（`base_branch`への実push、通常は`merge` jobによるSquash merge直後）: `Validate current commit (last commit)`ステップを実行する。このステップは、実際に生成された最終コミット（`HEAD~1`〜`HEAD`、Squash mergeコミットメッセージそのもの）を検証する。これは**マージ前のゲートではなく**（push自体がマージ完了後にしか発火しないため、原理的にゲートにはなり得ない）、別の目的を持つ。`reusable-cd.yml`の`release` jobは`base_branch`のコミット履歴をConventional Commitsとして解釈しバージョンを自動採番する。このため、**Squash mergeで実際に生成されたコミットメッセージがConventional Commits形式に従っているかを最終確認する**ためのチェックである。

`merge` jobはPRをsquash mergeする際、コミットタイトルを明示的に指定していない（`github.rest.pulls.merge`呼び出しに`commit_title`/`commit_message`を渡していない）。このためGitHubはデフォルトでPRタイトルをそのままmainに残る唯一のコミットの件名にする。かつて`Validate PR commits`（PR内の個々のコミットメッセージ）だけを検証していたときは、PRタイトル自体がConventional Commits形式（単一の有効なtype）に反していても素通りしていた。実際に、PRタイトルで複数のtypeを`/`で連結する（例:`fix/feat: ...`）とsquash merge後のcommit-analyzerがtypeを認識できずリリースが生成されない問題が発生した（[bamiyanapp/karuta#722](https://github.com/bamiyanapp/karuta/issues/722)）。`Validate PR title`ステップの追加により、この種の不整合は個々のコミット内容に関わらずマージ前のゲートで検知できるようになった（PRタイトルは第三者が自由に入力できる値のため、シェルへの直接展開ではなく`env:`経由でcommitlintへ渡し、コマンドインジェクションを防いでいる）。
...
- **run-name**: 参照側`cd.yml`の`on:`同様、`workflow_call`先のこのワークフロー自体では設定できないため、呼び出し元（参照側の`cd.yml`）で指定する。既定のrun-name（ワークフロー名のみ、または`workflow_run`トリガー時はトリガー元のコミットSHA）はGitHub ActionsのRun一覧上で実行内容が分からない。特にスマートフォンのブラウザでActionsタブを確認する運用（CLAUDE.md「開発環境の制約（スマホオンリー）」参照）では、どの変更に対応するCD実行かひと目で判別できない問題があった（[bamiyanapp/uchi-stock#337](https://github.com/bamiyanapp/uchi-stock/issues/337)）。`reusable-ci.yml`のCI側run-name規約（`CI (PR|push-to-main) <コミットメッセージ or PRタイトル>`）に倣い、トリガー方式に応じて以下のパターンを使う。
  - `push`トリガー: `github.event.head_commit.message`が使える。dev-standards自身の`cd.yml`（dogfooding）はこのパターン
    ...
    ```
  - `workflow_run`トリガー（CIの完了を待ってdeployする構成、参照側リポジトリの一般的な構成）を使う場合を考える。`github.event.workflow_run.head_sha`（コミットSHAのみ）ではなく`github.event.workflow_run.head_commit.message`（トリガー元のCI実行のコミットメッセージ）を使う
    ```yaml
    run-name: >-
...
- **実行内容**:
  ...
  - GitHub Pagesへのデプロイに限っては、`.github/actions/deploy-github-pages`（setup-node→npm ci→build→upload-pages-artifact→deploy-pages の定型パターンを共通化した複合action）を利用できる。呼び出し側の`cd.yml`は`environment: { name: github-pages }`・`permissions: { pages: write, id-token: write }`をjob単位で指定する。その上で、このactionを`with: working-directory / node-version / build-command / artifact-path`付きで呼び出す（`page-url`をoutputする）。参照側がnpm workspaces構成（[bamiyanapp/karuta#608](https://github.com/bamiyanapp/karuta/issues/608)）の場合は`workspaces: true`も併せて渡す。この場合`working-directory`はビルドコマンドの実行先ディレクトリのみを指し、依存インストール（`npm ci`）はリポジトリルートで行われる（`reusable-ci.yml`の同名inputと同じ意味）
  - Serverless Framework（osls等のCLIフォーク含む）を使ったAWSデプロイに限っては、`.github/actions/deploy-serverless`（setup-node→npm ci→デプロイコマンド実行の定型パターンを共通化した複合action）を利用できる。`with: working-directory / node-version / deploy-command / workspaces / aws-access-key-id / aws-secret-access-key`で呼び出す。npm workspaces構成かつ`package.individually: true`で多数のLambda関数を個別packagingする場合、`EMFILE: too many open files`が発生し得る（[bamiyanapp/karuta#608](https://github.com/bamiyanapp/karuta/issues/608), [#663](https://github.com/bamiyanapp/karuta/issues/663)）。この対策として、デプロイコマンド実行前にファイルディスクリプタのソフトリミットをハードリミットまで引き上げる処理をデフォルトで行う（`raise-fd-limit: false`で無効化可能）。上記2つ以外のデプロイ先は現状対象外で、参照側の`cd.yml`に個別実装する
...
- **実行内容**:
  - `analyze`（`languages`入力で指定した言語ごとに`matrix`展開、既定は`["javascript-typescript"]`）: `github/codeql-action/init` → `github/codeql-action/analyze` を実行する。結果はSARIFとしてGitHub Securityタブへアップロードする。JavaScript/TypeScriptはインタプリタ言語のため、コンパイル言語向けのautobuildステップは不要
...
`release` jobの`outputs.version`（算出済みのセマンティックバージョン、上記「2. CDワークフロー」参照）を使い、実際にデプロイされているビルドがどのバージョン・どの時点のものかをアプリの画面上に表示できるようにする運用パターン（examination#131）。「デプロイは成功したはずだが、画面には反映されていないように見える」といった問い合わせに対し、開発者に確認を依頼しなくてもユーザー自身が画面から確認できるようにする狙いがある。
...
1. **バージョン・SHA・ビルド日時を環境変数として用意する**: `release` jobの`outputs.version`は「このCD実行でsemantic-releaseが新バージョンを発行した場合のみ」設定される値である。`feat`/`fix`を含まないコミット（`docs`/`chore`等）による実行では**空文字列になる**点に注意（上記「2. CDワークフロー」の`outputs`参照）。「新バージョンが出た時だけ知りたい」用途（Slack通知等）であればこの値をそのまま使える。examinationのように**常にアプリの現在バージョンを表示し続けたい**用途では、`outputs.version`ではなく`package.json`の`version`フィールドを直接読む（`node -p "require('./package.json').version"`）。`release` jobは新バージョンを発行しない実行でも`package.json`の内容自体は変更しないため、直前のリリースバージョンがそのまま読み取れる。Git SHA・ビルド日時は参照側の`cd.yml`（`deploy` job等）内で1行で算出できる（例: `git rev-parse --short HEAD`、`date -u +%Y-%m-%dT%H:%M:%SZ`）。
   ```yaml
   deploy:
     needs: release
     steps:
...
3. **アプリ側で読み取り・表示する**: 各ビルドツールの環境変数アクセス方法（Viteなら`import.meta.env.VITE_APP_BUILD_VERSION`等）で読み取り、フッターや設定画面等に表示する。未設定時（ローカル開発時等）のフォールバック表示も用意しておくとよい。
...
`reusable-ci.yml` / `reusable-cd.yml` は上記の `git-submodules` 経由の固定運用とは別の参照経路も持つ。参照側の `ci.yml` / `cd.yml` から
...
dev-standards 自身も本ファイルが定める CD ワークフロー（`.releaserc.cjs` + `.github/workflows/cd.yml`、`reusable-cd.yml` を dogfooding で呼び出す構成）を使う。
...
タグ更新の検知には Renovate の `github-actions` マネージャーを使う。`config:recommended`（上記1の `renovate.json` が既に `extends` している）には`github-actions` マネージャーが標準で含まれているため、`git-submodules` のような追加の明示的な有効化は不要である。参照側リポジトリの `renovate.json` に変更を加えなくても、`uses: ...@vX.Y.Z` 形式の参照を Renovate が自動的に検知し、新しいタグが発行されるたびに更新 PR を作成する。
