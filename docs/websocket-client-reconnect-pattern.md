...
呼び出し側から渡されるコールバック（メッセージ受信時のハンドラ等）は、接続を確立する`useEffect`（または相当の初期化処理）の依存配列に含めると、コールバックの参照が変わるたびに不要な再接続が発生してしまう。コールバックは専用の`ref`に常に最新値を反映しておき、接続を確立する処理自体はURLや認証情報等、実際に再接続が必要な値にのみ依存させる。
