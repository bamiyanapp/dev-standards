...
通知音・BGM・読み上げ等、音声を非同期文脈（`fetch`完了後のコールバック、WebSocket通知の受信等）で再生するあらゆるWebアプリに共通する課題がある。Safari（iOS/macOS）の自動再生ポリシーへの対策パターンである。karuta（[bamiyanapp/karuta](https://github.com/bamiyanapp/karuta)）がベースである。具体的には`frontend/src/utils/audioUnlock.js`である。かるた固有の効果音の管理部分（`QUIZ_SFX_*`・`playQuizSfx`系）を除いた汎用部分を文章化したものである。CLAUDE.md「開発環境の制約（スマホオンリー）」が名指しする「検証端末はiPhone（iOS Safari）」という制約に直結する知見。
...
「ボタン押下→WebSocketで通知を受信→APIから音声データを取得→取得できたら`play()`」のような非同期処理を考える。その都度`new Audio(...)`していると問題が起きる。その要素自体は一度もユーザー操作中に再生されていないためSafariにブロックされ続けてしまう。
...
ユーザー操作の発生箇所（例: トップページのボタンクリック）と、実際に音声を再生する箇所（例: 別画面の非同期処理）が別のコンポーネントのライフサイクルにまたがることが多い。そのため、コンポーネントの`ref`ではなく**モジュールスコープのシングルトン**として保持する。
...
- テスト（vitest + jsdom）では、モジュールスコープのシングルトンがテスト間で共有されてしまうことがある。そうならないよう、`resetSharedAudioForTests`のようなテスト専用リセット関数を用意しておく
