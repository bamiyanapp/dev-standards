...
- `.github/actions/render-mermaid-diagrams/`: Markdown中の```` ```mermaid ```` ブロックを`@mermaid-js/mermaid-cli`でSVG画像へ事前レンダリングする複合action。`reusable-ci.yml`の`render-mermaid-diagrams` job（`enable_mermaid_render`入力）から利用する。
