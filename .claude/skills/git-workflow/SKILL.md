...
同じ作業ブランチに対してコミット・pushを複数回連続して行い、そのたびにPRを作成していると、直前のpushで作成した
...
1. **自己参照タグは自動更新されない。** 変更を含む新しいタグは、このPRのマージ・リリース後に初めて発行されるため、同じPR内で自己参照タグをその新タグへ更新できない（新規機能追加の場合はこの自己参照チェックアウト自体が無いため、いったん無効化した状態でリリースしてタグを発行し、フォローアップPRで有効化する「ブートストラップ」対応が必要になる。既存actionの内部実装変更の場合はブートストラップ問題は無いが、リリース後に自己参照タグを新タグへ更新するフォローアップPRが別途必ず必要）
