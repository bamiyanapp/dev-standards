...
このリポジトリ群はCIがgreenになると自動でPRをマージする運用である。1つの継続した作業セッション内で同じ作業ブランチに対してコミット・pushを複数回連続して行い、そのたびにPRを作成していると、直前のpushで作成したPRが自動マージされることがある。これは、次のコミットを準備している間（Verifier実行・レビュー等）に発生し、ローカルブランチが`origin/main`に対して古くなっていることがある。この状態に気づかずpushすると`mergeable_state: dirty`
（コンフリクト）になる。
