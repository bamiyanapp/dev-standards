...
1. main取り込み（`git fetch origin <base-branch>`で最新化し、作業ブランチへrebaseまたはmergeする）。長時間・複数コミットにまたがるセッションでは、作業開始時にmainを取り込んでいてもPR作成直前には既に古くなっていることがあるため、PR作成の直前に必ずこのステップをやり直す。**同一ブランチに対して同一セッション内で2回目以降のpushを行う場合も例外なくこのステップを毎回やり直す**（後述「同一ブランチへの連続pushにおける再確認」参照）。「前回確認したばかりだから」「直前のpushからまだ時間が経っていないから」は省略する理由にならない
2. 競合解消
3. push前に、対応するPRが既にマージ済み・クローズ済みでないか確認する（後述の「ブランチ再利用前の確認」）。
   マージ・クローズ済みの場合はそのブランチへ追加コミットをpushしてはならない。
4. push
5. PR（MR）作成: ユーザーへ確認せず自動で行う。本項目自体が事前承認であり、都度作成の可否を尋ねない。同一の作業ブランチに対応する**オープンな**PRが既に存在する場合は新規作成せずpushのみで更新する（承認・マージの扱いは下記「PR（MR）承認・マージ禁止（強制）」を参照）。作成後、`mergeable_state`を確認する。`dirty`（コンフリクト）であれば直ちに1へ戻りmain取り込み・競合解消をやり直してから再度pushする（コンフリクトしたままユーザーへ完了報告してはならない）
6. main最新化
...
1. **自己参照タグは自動更新されない**。変更を含む新しいタグは、このPRのマージ・リリース後に初めて発行されるため、同じPR内で自己参照タグをその新タグへ更新できない。新規機能追加の場合はこの自己参照チェックアウト自体が無いため、いったん無効化した状態でリリースしてタグを発行し、フォローアップPRで有効化する「ブートストラップ」対応が必要になる。既存actionの内部実装変更の場合はブートストラップ問題は無いが、リリース後に自己参照タグを新タグへ更新するフォローアップPRが別途必ず必要である
