...
- ただし、これらの破壊的操作はシステムの Git Safety Protocol（force push、`reset --hard`、未コミット変更の上書き等）に従い、必要な場面では事前にユーザーへ確認する。本節はあくまで「意図した操作が対話プロンプトで止まらないようにする」ためのものであり、確認手順を省略する許可ではない。
